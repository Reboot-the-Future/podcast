  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-3 xs:mb-4 sm:mb-5">
        ...
        >
          <Sparkles className="text-[#FFA9FC]" size={14} />
      ...
      <div className="flex items-center gap-2 xs:gap-3 sm:gap-5">
        ...
        <div className="flex-1 min-w-0 relative">
          ...
          >
            <div
              className="absolute top-0 left-0 h-full bg-[#FFA9FC] transition-all rounded-full"
              style={{ width: `${progress}%` }}
            />
            {/* Progress indicator circle */}
            <div
              className={`absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-[#FFA9FC] rounded-full shadow-lg border-2 transition-all hover:scale-110 ${isDark ? 'border-[#0F1C1C]' : 'border-white'}`}
      ...
      <div
        className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 xs:gap-3 sm:gap-0 mt-3 xs:mt-4 pt-3 ${isDark ? 'border-[#FFA9FC]/20 border-t-2' : 'border-gray-200/50 border-t'
          }`}
      >
        <div className="flex items-center gap-1.5 xs:gap-2 sm:gap-3">
          ...
          <input
            ...
            className="w-16 xs:w-20 sm:w-28 accent-[#FFA9FC]"
            aria-label="Volume"
          />
        </div>
        ...
        >
          <div
            className={`w-2 h-2 rounded-full ${isPlaying ? 'bg-[#FFA9FC] animate-pulse' : 'bg-gray-400'
              }`}
          />
...
const BlogSidebar = ({ blogs, currentPage, isDark }: { blogs: BlogItem[]; currentPage: number; isDark: boolean }) => {
  ...
  return (
    <div className="flex flex-col space-y-3 xs:space-y-4 sm:space-y-6">
      ...
      <div
        className={`relative rounded-2xl sm:rounded-3xl overflow-hidden ${isDark ? 'bg-[#0F1C1C] border-[#FFA9FC]/20 border-2' : 'bg-white border border-gray-200 shadow-lg'
      ...
      >
        <div className="relative z-10">
          <div className="flex items-center gap-1.5 xs:gap-2 sm:gap-3 mb-3 xs:mb-4 sm:mb-6">
            <div className="w-7 h-7 xs:w-8 xs:h-8 sm:w-10 sm:h-10 rounded-full bg-[#FFA9FC] flex items-center justify-center shadow-lg">
            ...
            <div className="space-y-2.5 xs:space-y-3 sm:space-y-4">
              {blogs.map((blog) => {
                ...
                return (
                  <div
                    key={blog.id}
                    className={`flex flex-col gap-1.5 xs:gap-2 ${isDark ? 'border-[#FFA9FC]/20' : 'border-gray-200'
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1 xs:gap-1.5 sm:gap-2">
                      {blog.tags.slice(0, MAX_VISIBLE_TAGS).map((tag) => (
                        <span
                          key={tag}
                          className={`text-xs text-[#FFA9FC] ${isDark ? 'bg-[#FFA9FC]/10' : 'bg-[#FFA9FC]/20'
                            } px-1.5 xs:px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full border border-[#FFA9FC]/20 hover:bg-[#FFA9FC]/20 transition-colors`}
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    {shouldTruncate && (
                      <button
                        ...
                        className="text-xs text-[#FFA9FC] hover:underline underline-offset-2 font-semibold transition-colors text-left mt-0.5 xs:mt-1"
                      >
...
const EpisodeCard = ({ episode, isDark }: { episode: Episode; isDark: boolean }) => {
          ...
          <div className="flex gap-1 xs:gap-1.5 sm:gap-2 flex-wrap mb-1.5 xs:mb-2 sm:mb-3">
            {tags.slice(0, MAX_VISIBLE_TAGS).map((tag: string) => (
              <span
                key={tag}
                className={`text-xs text-[#FFA9FC] ${isDark ? 'bg-[#FFA9FC]/10' : 'bg-[#FFA9FC]/20'
                  } px-2 xs:px-2.5 sm:px-3 py-0.5 xs:py-0.5 sm:py-1 rounded-full font-semibold border border-[#FFA9FC]/20`}
              >
                {tag}
              </span>
            ))}
  ...
  <div className={`${isDark ? 'text-[#efe8e6]' : 'text-[#0F1C1C]'} mb-2 xs:mb-3 sm:mb-4 flex-grow text-xs xs:text-sm sm:text-base break-words`}>
      ...
      <div
        className={`flex flex-wrap items-center justify-between gap-1.5 xs:gap-2 sm:gap-0 pt-2 xs:pt-3 sm:pt-4 ${isDark ? 'border-[#FFA9FC]/20 border-t-2' : 'border-gray-200 border-t'
          } mt-auto`}
      >
      ...
      {episode.buzzsprout_episode_id && (
        <div
          className={`mt-3 xs:mt-4 sm:mt-6 pt-3 xs:pt-4 sm:pt-6 ${isDark ? 'border-[#FFA9FC]/20 border-t-2' : 'border-gray-200 border-t'} space-y-3 xs:space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-top-4 duration-300`}
        >
          <div
          ...
          >
            <div className="flex items-center gap-1.5 xs:gap-2 sm:gap-3 mb-3 xs:mb-4 sm:mb-5">
              <div className="w-8 h-8 xs:w-10 xs:h-10 sm:w-12 sm:h-12 rounded-full bg-[#FFA9FC] flex items-center justify-center shadow-lg">
            ...
            <div id={containerIdRef.current} className="buzzsprout-player-wrapper w-full overflow-hidden bg-white rounded-lg" aria-busy="true">
              ...
              >
                <div className="animate-spin rounded-full h-5 w-5 xs:h-6 xs:w-6 sm:h-8 sm:w-8 border-b-2 border-[#FFA9FC] mx-auto mb-2 sm:mb-3"></div>
...
export default function PodcastSite() {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#0F1C1C] text-white' : 'bg-white text-[#0F1C1C]'}`}>
      <header
        className={`sticky top-0 ${isDark ? 'bg-[#0F1C1C]/95 border-[#FFA9FC]/20 border-b-2 backdrop-blur-md' : mobileMenuOpen ? 'bg-white border-[#FFA9FC]/30 border-b-2' : 'bg-white/95 border-[#FFA9FC]/30 border-b-2 backdrop-blur-md'
        ...
        <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 sm:py-3 lg:py-4">
          ...
          {mobileMenuOpen && (
            <>
              ...
              {/* Mobile Menu */}
              <nav
                className={`lg:hidden mt-4 pb-4 space-y-2 border-t-2 ${isDark ? 'border-[#FFA9FC]/20' : 'border-[#FFA9FC]/30'
      ...
      <section className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 pt-8 sm:pt-12 lg:pt-16 pb-4 sm:pb-6">
        <img 
          ...
          className={`w-full h-auto object-contain rounded-2xl sm:rounded-3xl ${isDark ? 'border-2 border-[#FFA9FC]/20' : ''}`}
        />
      </section>
          ...
          >
            <div className="relative z-10">
              <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#FFA9FC] flex items-center justify-center shadow-lg">
      ...
      <section className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 py-4 xs:py-6 sm:py-12 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-[3fr_1fr] gap-4 sm:gap-6 lg:gap-10">
          ...
          <div
            className={`relative rounded-2xl sm:rounded-3xl h-full min-h-[500px] xs:min-h-[600px] sm:min-h-[650px] order-1 ${isDark ? 'bg-[#0F1C1C] border-[#FFA9FC]/20 border-2 overflow-hidden' : 'bg-white border border-gray-200 shadow-lg'
                ...
                <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="flex gap-1 xs:gap-1.5 sm:gap-2 flex-wrap">
                      {parseTags(heroEpisode.tags).slice(0, 5).map((tag: string) => (
                        <span
                          key={tag}
                          className={`text-xs text-[#FFA9FC] ${isDark ? 'bg-[#FFA9FC]/10' : 'bg-[#FFA9FC]/20'
                            } px-2 xs:px-2.5 sm:px-4 py-0.5 xs:py-1 sm:py-1.5 rounded-full font-semibold border border-[#FFA9FC]/30 hover:bg-[#FFA9FC]/20 transition-colors`}
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="flex-1 flex flex-col justify-center space-y-3 xs:space-y-4 sm:space-y-6 my-3 xs:my-4 sm:my-8">
                    <div className="flex-1 flex flex-col justify-start mt-2 sm:mt-3 w-full">
                      <h1
                        className={`font-bold text-lg xs:text-xl sm:text-2xl md:text-3xl lg:text-[34px] xl:text-[46px] w-full ${isDark ? 'text-white' : 'text-[#0F1C1C]'} mb-4 xs:mb-5 sm:mb-6`}
                        style={{ wordBreak: 'keep-all', overflowWrap: 'break-word', lineHeight: '1.35' }}
                      >
                        {heroEpisode.title}
                      </h1>
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm">
                      ...
                      >
                        <Clock className="text-[#FFA9FC] w-3.5 h-3.5 xs:w-4 xs:h-4 sm:w-[18px] sm:h-[18px]" />
                        ...
                        >
                          <div className="flex flex-col items-center justify-center">
                            <div className="relative mb-3 xs:mb-4">
                              <div className={`w-10 h-10 xs:w-12 xs:h-12 sm:w-16 sm:h-16 rounded-full ${isDark ? 'bg-[#FFA9FC]/10' : 'bg-gray-100'
                                } flex items-center justify-center`}>
                                <Play className="text-[#FFA9FC] w-5 h-5 xs:w-6 xs:h-6 sm:w-8 sm:h-8" fill="#FFA9FC" stroke="none" />
                              </div>
                              <div className="absolute inset-0 rounded-full border-2 border-[#FFA9FC]/30 animate-pulse"></div>
                            </div>
      ...
      <section id="episodes" className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 py-4 xs:py-6 sm:py-12 lg:py-16">
        ...
        >
          <div className="relative z-10">
            <div
              className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 pb-6 sm:pb-8 ${isDark ? 'border-[#FFA9FC]/20 border-b-2' : 'border-b border-gray-200'
                }`}
            >
              ...
              <div
                className={`flex items-center gap-2 sm:gap-4 flex-shrink-0 ${isDark ? 'border-[#FFA9FC]/20 border-2' : ''
                  } rounded-lg p-2`}
              >
                <span
              ...
              <div className="flex justify-center items-center py-16 sm:py-20">
                <div className="text-center">
                  <div className="relative mb-4 sm:mb-6">
                    <div className="animate-spin rounded-full h-10 w-10 sm:h-12 sm:w-12 border-b-2 border-[#FFA9FC] mx-auto"></div>
                    <div className="absolute inset-0 animate-pulse rounded-full h-10 w-10 sm:h-12 sm:w-12 border border-[#FFA9FC]/30 mx-auto"></div>
                  </div>
            ...
            {!loading && currentPageEpisodes.length > 0 && (
              <div
                className={`flex justify-center gap-2 mt-6 sm:mt-8 sm:hidden ${isDark ? 'border-[#FFA9FC]/20 border-2 bg-[#0F1C1C]' : 'bg-gray-100'
                  } rounded-lg p-2`}
              >
      ...
      <footer
        className={`${isDark ? 'bg-[#0F1C1C] border-[#FFA9FC]/20 border-t-2' : 'bg-white border-gray-200 border-t'} mt-8 sm:mt-16 lg:mt-20`}
      >
